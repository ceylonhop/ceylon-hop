...
export function quoteBreakdown(req: QuoteRequest): QuoteBreakdown {
  ...
  const vehicle: Vehicle = pricedVehicle(req);
  ...
  const perKm = override ?? RATE_CARD.perKmCents[vehicle];
  const src =
  ...
  const legs: LegBreakdown[] = src.map((l) => {
    ...
    return { from: l.from, to: l.to, distanceKm: l.distanceKm, billableKm: bKm, priceCents: legPriceCents(bKm, vehicle, override), cls: vehicle, minApplied };
  });
