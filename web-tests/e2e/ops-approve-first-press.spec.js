...
// Approve used to do NOTHING on the first press right after opening a quote for review
...
test('Approve works on the FIRST press right after opening a quote for review', async ({ page }) => {
  const rec = { patches: [] };
  await stub(page, rec);
  ...
  // Open the in-review quote, then press Approve immediately — while the re-price this triggers
  // is still in flight. That is the whole point: no settling wait before the press.
  await page.evaluate(() => QuoteView.openQuote('q_review'));
  await page.waitForSelector('#quoteRoot [data-action="approveReady"]', { timeout: 10000 });
  await page.locator('#quoteRoot [data-action="approveReady"]').dispatchEvent('click');

  // The transition must go through on that first press.
  await expect.poll(() => rec.patches.length, { timeout: 8000 }).toBeGreaterThan(0);
  expect(rec.patches[0].status).toBe('ready');

  // …and the operator must never have been shown a price blocker that then self-healed.
  await expect(page.locator('#quoteRoot .ch-blockers, #quoteRoot [data-action="dismissBlockers"]')).toHaveCount(0);
});
