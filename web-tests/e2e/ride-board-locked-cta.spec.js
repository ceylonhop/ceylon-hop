...
// The row CTA (a card CTA until 2026-09-18) used to invite a hop-on on every ride the board
// draws as locked. "Locked" on a row is `L.confirmed || committed >= minSeats`, which is two different
...
test('a confirmed van invites you to look, not to join', async ({ page }) => {
  ...
  const card = page.locator('.rw[data-code="LK-1111"]');
...
test('a van that has merely hit its minimum still invites you on', async ({ page }) => {
  ...
  const card = page.locator('.rw[data-code="GA-3333"]');
  await expect(card).toBeVisible({ timeout: 15000 });
