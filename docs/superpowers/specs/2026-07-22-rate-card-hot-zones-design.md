...
**Status (corrected 2026-08-09):** spec fully resolved on 2026-07-22 (revision 4 below). **Phases 1, 2 and 4's polish have SHIPPED** — PRs #116 (dormant plumbing + founder admin) and #126 (applied-zone premium chip + leg-chip parity). Live in code: `api/src/quote/hotZones.ts`, the `pricing_zones` table, `postgresZonesRepo`, the founder admin panel, and the zone-aware cost basis (`perRideBoost` in `engine.ts:63`, `boostedBillableKm` at `:96`, per D6). **Phase 3 (§8.3) is partially shipped:** as of 2026-08-12 (this branch), `bookings.ts` and the public quote routes now compose zones — only `generate-pricing.mjs`/the front-end mirror remain zone-less, so with any zone active, an ops quote and a website quote for the same trip still price differently, exactly as §8.2 predicted and accepted.
...
**Phase 3 is a prerequisite for public automatic promotions** — see `2026-07-15-discounts-design.md` §18.2. Founder manual discounts (phase 1 of that feature) are unaffected.
