...
the ops builder, a founder-only read endpoint, and a minimal history panel (a list — no diff view).
...
| `updated_by` | `text` | staff email; null for the customer web flow |
...
`GET /admin/quote/:id/revisions` — founder-only (`quote:manage` + the founder gate the margin
endpoints already use). Returns newest-first:

```
{ revision, totalCents, currency, updatedBy, createdAt, status, changed: string[] }
```

`changed` is computed server-side by comparing consecutive versions, and names **fields, not
values**: `legs`, `stops`, `distance`, `extras`, `vehicle`, `pax`, `bags`, `dates`, `total`.
"Extras changed, total changed" is what turns yesterday's mystery into a sentence.
...
**History panel:** founder-only, in the quote builder. A list — revision, when, who, total, and the
changed-field chips. No diff viewer. Without any UI, "so we know when something goes wrong" means
running SQL by hand, which is roughly today's situation.
...
| column | meaning |
| --- | --- |
| `customer_total_cents` | the total as of the last customer-facing moment; null if never |
...
⚠️ **The mint currently patches only when the selection changed** (partial-leg links, spec
2026-08-04). A plain full-total mint writes nothing today, so it must stamp the marker regardless —
otherwise the most common way a customer sees a price leaves no baseline.
...
- **Atomicity:** a failing snapshot rolls the content write back (no orphan revision).
...
- **Wire:** `/revisions` carries no margin, hot-zone or rate-card field; non-founders get 403.
...
Additive migration (one new table, three nullable columns), additive endpoint, additive UI. Every
existing quote simply has no history until its next save, and no baseline until its next send.
`main` → staging automatically; migrations apply on boot, fail-closed, so merging the migration is
its release. Prod follows via the usual promote PR.
