...
- [~] M7 Connect the live website — flag-gated (`window.CEYLON_HOP_API` / `?api=`; default = simulated). Single ✓ (verified browser→API→Supabase). **Trip + shared flows now wired** (API resolves the shared corridor from from/to). Live browser verification of trip/shared is **pending the DB restore** (`.env` truncation). 7.2 confirmation-reads-booking + full staging click-through still open.
