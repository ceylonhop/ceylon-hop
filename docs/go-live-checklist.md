...
- [ ] **Apply all pending DB migrations on every deploy (`cd api && npm run migrate`) — REQUIRED; code ships ahead of schema otherwise.** `drizzle-kit migrate` is a **manual** step (not run automatically on Render deploy), so a feature's columns can be live in code while missing from the prod DB. As of 2026-07-12 the DB must be at **0014** (`rate_card_json` + `rate_locked_until`, the rate-lock columns). **Symptom when a migration is missed (observed 2026-07-12): opening or editing any quote — and checking out from a saved web quote — 500s with `internal_error`,** because `postgresQuoteRepo.get()` / `bookings.ts` select the full `quotes` row and reference columns the DB lacks; the quote *list* uses a narrow select so it still renders, which masks the cause. **Fix: run `npm run migrate` against prod. Prevent recurrence: fold `npm run migrate` into the deploy runbook/CI so schema and code always ship together.**
