...
- [ ] **Serve the new site on the `ceylonhop.com` apex** (currently the live business site). Required because PayHere only works on the apex.
  ...
  - **Founder follow-ups (non-blocking, flagged during M16):** route pages ship with a CSS-gradient hero and **no photos** — supply ~10–20 route images for the P2 image-SEO win; the ported **privacy policy is thin** (3 bullets from the old site) and the **terms reference the old "travel pass" hop-on/hop-off model** — both should be reviewed/expanded against the new booking/transfer product before real customers; the 6 old blog posts currently 301 to `blog.html` (re-port later if desired).
