...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      // A non-passive wheel listener forces every wheel event over the slot onto
      // the main-thread scroll path, which staggers page scrolling over large
      // slots. So the handler is only ATTACHED while reframing (editor-only) and
      // detached on exit — see _enterReframe / _exitReframe.
      this._onWheel = (e) => {
    ...
    _enterReframe() {
      ...
      // Wheel-zoom only matters in reframe mode; attach the non-passive listener
      // now so normal browsing keeps the fast (passive) scroll path.
    ...
    _exitReframe(commit) {
      ...
      this.removeAttribute('data-panning');
      this.removeEventListener('wheel', this._onWheel, { passive: false });
